...
from .bw_exo_amp_env_cfg import BwExoAmpEnvCfg
from ..motions.python import MotionLoader


class BwExoAmpEnv(DirectRLEnv):
    cfg: BwExoAmpEnvCfg

    def __init__(self, cfg: BwExoAmpEnvCfg, render_mode: str | None = None, **kwargs):
...
def feet_slipping(
    ...
    # return -(threshold - feet_too_near_humanoid(joint_pos)).clamp(min=0)
@torch.jit.script
def compute_rewards(
    ...
    total_reward = rew_termination + rew_action_l2 + rew_joint_pos_limits + rew_joint_acc_l2 + rew_joint_vel_l2 +  rew_distance + rew_slip
