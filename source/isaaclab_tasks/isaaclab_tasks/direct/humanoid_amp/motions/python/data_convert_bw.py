...
def main():
    ...
    # csv_file = "../bw/test.csv"
    csv_file = "../g1/walk1_subject1_delete.csv"
    ...
    # # 3.2 定义原始采样率 (90fps) 和新采样率 (60fps)，构造时间序列
    fps_orig = 30
    ...
    # fps_new = 60
    # dt_new = 1.0 / fps_new
    # N_new = int(np.ceil(fps_new / fps_orig * N_orig))
    # t_new = np.linspace(0, (N_orig - 1) * dt_orig, num=N_new, endpoint=True)  # ✔️ 精确匹配原始时间范围
# 3.3 对 root_data 的位置和关节角度进行插值
    # 位置（前三列）采用线性插值
    root_pos_interp = interp1d(t_orig, root_data_orig[:, 0:3], axis=0, kind='linear', fill_value="extrapolate")(t_new)
    ...
    joint_data = interp1d(t_orig, joint_data_orig, axis=0, kind='linear', fill_value="extrapolate")(t_new)
    ...
    mesh_dir = "../robot_description/bw/meshes"
    robot = build_pin_robot(urdf_path, mesh_dir)
    model = robot.model
    data_pk = robot.data

    # 在 root_data 插值后，插入以下代码
    z_offset = 0.5 
    ...
    # out_filename = "../bw_walk_npy/bw_self.npz"
    out_filename = "../bw_walk_npy/bw.npz"
